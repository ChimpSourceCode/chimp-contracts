...
contract CHIMPStakingReserve is Ownable {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;
    mapping(address => bool) public operators;

    IERC20 public chimpToken;

    constructor(IERC20 _chimpToken) {
        chimpToken = _chimpToken;
    }
    ...
    function getCHIMPBalance() public view returns (uint256) {
        uint256 balance = chimpToken.balanceOf(address(this));
        return balance;
    }

    function withdrawCHIMP(address to, uint256 amount) public {
        require(operators[msg.sender] == true, "not allowed");
        uint256 balance = getCHIMPBalance();
        if (balance < amount) {
            chimpToken.transfer(to, balance);
        } else {
            chimpToken.transfer(to, amount);
        }
    }
}

contract CHIMPMasterChef is Ownable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    ...
    CHIMPStakingReserve public chimpStakingReserve;
    ...
    constructor(
        ...
        CHIMPStakingReserve _CHIMPStakingReserve,
        uint256 _startBlock
    ) {
        chimp = _chimp;
    ...
    function pendingCHIMP(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 chimpReward = multiplier
                .mul(chimpPerBlock)
                .mul(pool.allocPoint)
                .div(totalAllocPoint);
            accCHIMPPerShare = accCHIMPPerShare.add(
                chimpReward.mul(1e12).div(lpSupply)
            );
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        refillCHIMP(address(this), chimpReward);

        pool.accCHIMPPerShare = pool.accCHIMPPerShare.add(
            chimpReward.mul(1e12).div(lpSupply)
        );
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for CHIMP allocation.
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (_amount > 0) {
            ...
            if (pool.depositFeeBP > 0) {
                ...
                pool.tvl += _amount.sub(depositFee);
            } else {
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        updatePool(_pid);
        uint256 pending = user.amount.mul(pool.accCHIMPPerShare).div(1e12).sub(
            user.rewardDebt
        );
        if (pending > 0) {
            safeCHIMPTransfer(msg.sender, pending);
    ...
    function refillCHIMP(address _to, uint256 _amount) internal {
        chimpStakingReserve.withdrawCHIMP(_to, _amount);
    ...
    function updateEmissionRate(uint256 _chimpPerBlock) public onlyOwner {
        massUpdatePools();
        chimpPerBlock = _chimpPerBlock;
    }
}
